...
本システムの観測点名・市区町村名などの地名ローマ字変換は、**日本郵便が公開する郵便番号データ**をベースとしている。

- 日本郵便 郵便番号データダウンロード: https://www.post.japanpost.jp/service/search/zipcode/download/utf-zip.html

郵便番号データに含まれる「日本語地名（読み）」を取得し、それをローマ字に変換することで、地名の英語表記を生成している。

気象庁の多言語辞書を優先し、併用することでより正確な英語表記送信を実現している。
